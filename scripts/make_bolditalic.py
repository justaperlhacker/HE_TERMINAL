#!/usr/bin/env python3
"""Create a Bold Italic companion for HE_TERMINAL.

Usage: python3 make_bolditalic.py [slant_degrees]
Reads HE_TERMINAL-Bold.ttf, writes HE_TERMINAL-BoldItalic.ttf.
Same shear as make_italic.py so the two italics match.
"""
import math
import sys
...
SRC = "HE_TERMINAL-Bold.ttf"
DST = "HE_TERMINAL-BoldItalic.ttf"
...
def main() -> None:
    ...
    for rec in name_tbl.names:
        if rec.nameID == 2:
            rec.string = "Bold Italic"
        elif rec.nameID == 4:
            rec.string = "HE_TERMINAL Bold Italic"
        elif rec.nameID == 6:
            rec.string = "HE_TERMINAL-BoldItalic"

    os2 = f["OS/2"]
    os2.usWeightClass = 700
    os2.fsSelection = (os2.fsSelection & ~0x40) | 0x21  # REGULAR off; BOLD+ITALIC on
    f["head"].macStyle |= 0x0003

    f.save(DST)
    print(f"wrote {DST} ({angle:.1f} deg slant on bold)")
